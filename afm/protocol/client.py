...
import afm.protocol.rencode as rencode
from afm.protocol import ClientContextFactory
...
class DelugeRPCRequest(object):
    ...
    def format_message(self):
        ...
        """
        if self.request_id is None or self.method is None or \
                                    self.args is None or self.kwargs is None:
            raise TypeError("You must set the properties of this object before "
                            "calling format_message!")

        return (self.request_id, self.method, self.args, self.kwargs)

class DelugeRPCProtocol(Protocol):
    factory = protocol = None
...
class DelugeRPCClientFactory(ClientFactory):
    ...
    def clientConnectionFailed(self, connector, reason):
        log.warning("Connection to daemon at %s:%s failed: %s", connector.host,
                    connector.port, reason.value)
...
class DaemonSSLProxy(DaemonProxy):
    ...
    def deregister_event_handler(self, event, handler):
        ...
        """
        if event in self.__factory.event_handlers and \
                            handler in self.__factory.event_handlers[event]:
...
#            return True
    ...
    def is_localhost(self):
        ...
        """
        if self._daemon_proxy and \
                    self._daemon_proxy.host in ("127.0.0.1", "localhost") or \
